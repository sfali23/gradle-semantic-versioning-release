...
Determine and sets the next applicable version based on configuration.

=== printVersion

Prints the next applicable version based on configuration.

=== createTag

Creates `git` tag. The `setReleaseVersion` task must be called before calling this task.

=== pushTag

Pushes the tag created by `createTag` task.

[IMPORTANT]
====
This is *NOT* a _publish_ plugin. It must be used alongside _publish_ plugin of your choice.

For example, this project used `com.vanniktech.maven.publish` to publish artifacts to `Maven` central. It runs tasks in the following order:

[source, shell]
----
    ./gradlew setReleaseVersion publishToMavenCentral createTag pushTag
----
====
...
semverrelease {
    ...
    releaseTagComment.set("Releasing")
}
----
...
The default comment of the release tag is `Release <version_to_be_released>`, this setting allows to add un-released commits to tag summary. With this setting on tag comment will be:
